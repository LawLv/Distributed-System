...
init_slave(Id, Grp, Master) ->
    ...
    receive
        {view, [Leader|Slaves], Group} ->
            erlang:monitor(process, Leader), % start monitoring leader
            Master ! {view, Group},
...
leader(Id, Master, Slaves, Group) ->
    receive
        {mcast, Msg} ->
            ...
            Master ! Msg,
            leader(Id, Master, Slaves, Group);
        {join, Wrk, Peer} -> % {join, app, node id}
...
slave(Id, Master, Leader, Slaves, Group) ->
    receive
        ...
        {'DOWN', _Ref, process, Leader, _Reason} -> % {_, _, _, Leader ID, _ }
            election(Id, Master, Slaves, Group);
        stop ->
            ok
    end.

election(Id, Master, Slaves, [_|Group]) -> % rest of the group
    Self = self(),
    case Slaves of
        [Self|Rest] ->
